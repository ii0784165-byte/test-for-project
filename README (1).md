...
├── config/
│   ├── config.example.yaml
│   └── config.yaml          # git-ignored, holds your real keys
├── cron/
│   └── crontab_example.txt
├── .gitignore
├── requirements.txt
...
> ⚠️ **Security note:** Real API keys and tokens must **never** be committed to the repository. Make sure `config/config.yaml` is listed in `.gitignore` **before** you create it — the default Python `.gitignore` template does *not* exclude it by default, so add the following line yourself if it's missing:
> ```gitignore
> config/config.yaml
> ```
...
**2. Fill in your own credentials in `config/config.yaml`** (this file stays local only):
...
opencti:
  url: "YOUR_OPENCTI_INSTANCE_URL"     # e.g. http://localhost:8080
  token: "YOUR_OPENCTI_TOKEN"
```

**3. Verify `config/config.yaml` is git-ignored:**
...
Only `config/config.example.yaml` (with placeholder values, no real secrets) should ever be committed.
...
- **RamzitheStudent** — Holberton School Student
- *(add teammates here)*
